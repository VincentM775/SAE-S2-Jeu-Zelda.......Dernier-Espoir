...
public class CocktailM extends Projectile{
    ...
    public CocktailM(int degats, Environnement environnement, Acteur acteurQuiALancer) {
        ...
        this.arriver = new SimpleBooleanProperty(false);
    }
        ...
        for (int y = -1; y <= 1; y++) {
            for (int x = -1; x <= 1; x++) {
                if (tuileAligne + y >= 0 && tuileAligne + y < getEnvironnement().getInfoTuile()[1] && tuileAcolonne + x >= 0 && tuileAcolonne + x < getEnvironnement().getInfoTuile()[1]) {
                    getEnvironnement().getTerrain().getTerrain().remove(caseAEnflamer(y, x)); //case à remplacer selon x et y
                    getEnvironnement().getTerrain().getTerrain().add(caseAEnflamer(y, x), 2); //case à remplacer selon x et y
                }
            }
        }
        setArriver(true);
...
//         et même choses côté vue
        ...
        public int caseAEnflamer(int caseY,int caseX){
            return (((getX()/getEnvironnement().getInfoTuile()[0])+caseY)*getEnvironnement().getInfoTuile()[1]+(getY()/getEnvironnement().getInfoTuile()[0])+caseX);
        }

    public boolean getArriver() {
        return arriver.getValue();
    }

    public void setArriver(boolean arriver) {
        this.arriver.set(arriver);
    }

    public BooleanProperty arriverProperty() {
        return arriver;
    }
}
